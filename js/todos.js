...
class Todos extends Matreshka.Array {
	...
	constructor() {
		...
		Matreshka.initRouter(this, '/state/');
	}
	...
	events() {
		return this
			// Add an event handler to the changing of the ``"JSON"`` property which keeps the representation of the todo list as JSON string. In order to access a hard drive as rare as possible (because it works slower than RAM), the [onDebounce](http://matreshka.io/#!Matreshka-onDebounce) method is used, it prevents a multiple invocation of a handler over a period of time.
