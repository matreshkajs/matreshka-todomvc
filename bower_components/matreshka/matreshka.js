...
/*
 DONE:
	Fixes in balalaika for old WebKits such as iOS 5 WebKit
	"on" as function in binder (previous release)
	removed itemrender event
	used MK.Object#renderer as alternative to MK.Array#itemrenderer
	experimental MK.Array#renderIfPossible property
	experimental MK.Object#bindRendered
	lookForBinder returns undefined if no binder is  found
	rerender instead of initializeSmartArray
	renamed initDomItem, killDomItem to _renderOne and _destroyOne
	"#this" key instead of "__this__"
	private _trigger method
	optimized inner event triggers
	optimized MK.Array#initMK using _on method instead of on method
	added experimental addone and removeone events for MK.Array
	Fixed bug: DOM event handler executes few times
	randomString
	"_this_" key instead of "#this"
	MK.version
	Throw error when use deprecated "#this" or "__this__" keys
	Service method MK.warnDeprecated
	Return length from MK.Array#push and MK.Array#unshift methods (original methods behavior)
	Marked as deprecated MK.Array methods: silentPush, silentSplice, solentSort etc
	Created original array methods that accept event options (push_, sort_, splice_ etc)
	Moved lookForBinder to MK namespace
	Fixed issue in input[type="text"] default binder
	Added default binder ofr select[multiple]
	Created private MK#_initMK method
	Removed MK#initMK method
	Trigger MK.Array events only if array was changed
	addDependency: many keys of another instance [ inst, inst.keys() ]
	Delegated DOM events: this.on( 'click::something(.x > .y)' ) (experimantal)
	Return this from MK#off
	MK.procrastinate -> MK.debounce
	"private" global event option
	Removed Matreshka#defineNotEnum method 
	"silentHTML" and "skipDependencies" event options for MK#set function
	bindRendered for MK.Array
	MK.deprecatedNotifier
	setMediator -> mediate
	bindElement -> bindNode
	unbindElement -> unbindNode
	addDependency -> linkProps
	setItemMediator -> mediateItem
	addJSONKeys -> addDataKeys
	removeJSONKeys -> removeDataKeys
	Removed deprecated methods from docs
	Matreshka.Array#renderer could be a string
	Added "self" key to Array events
	Dynamically change Model
	Try to call toJSON before mediate array item and create new MK.Object
	Fixed bug: delegated events didn't work for MK.Object instances
	Call change event when binding changes property value by node default value (can be canceled via assignDefaultValue:false option)
	Always return null from MK#bound if element is not found
	Optimized MK.Object code a little
	Fixed unobvious bug in MK#set function (mediateITem + linkProps)
	Fixed bug in MK#defineGetter
	Fixed bug in MK.Array#concat
	Call MK#renderer by Matreshka.Array in own context
	Removed Number.isNaN polyfill
	Created private method MK#_getNodes
	Extended selectors system with :sandbox and :bound() selectors
	Fix in _getNodes()
	attributes support for balalaika.create function
	xclass.fn key (xclass.prototype)
	Experimental simple template engine
	Fixed bug in MK#once
	Removed binders that used rarely
	Moved default binders to mk.binders by creating named binders
	MK#immediate function
	MK#onDebounce
	MK#bindOptionalNode
	Fixed itemMediator bug in MK.Array
	Added skipMediator flag for MK.Array methods
	Allowed to add short delegated DOM events (click::(.selector) instead of click::_this_(.selector))
	MK.Array#pull accepts object
	Improved _parseBindings
	Allow to use :bound(x) > .s selectors
	Render elements even if Model is added dynamically
	Fix in _destroyOne and _renderOne (inner fix)
	Minor fixes in bindNode
	Use delete event instead of remove in MK Object (bug)
	Remove event by off method if it's added by onDebounce method (bug)
	Fixed bug: setValue calls on changed element even if value is same
	Fixed bug: bound nodes don't update after mediate function
	delay instead of immediate
	rewritten most of binders from MK.binders
	many fixes for IE8
	sandbox instead of _this_
	marked all _this_ strings as deprecated
	MK.throwDeprecated
	Improved parser,
	MK.Array#useParser
	for..of support for MK.Array and MK.Object
	
 TODO:
	Fire remove and modify when data key is removed from MK.Object instance (remove, removeDataKeys)
	"force" flag for MK.Array events
	MK.toMK method
	MK#off for delegated DOM events with specified selector
	Refactor MK#_on and MK#_off methods
	How to get parent in delegated event?
	MK.Array,MK.Object#toJSONString?
	Optimization: remove event after call handled by MK#once
	destroy function for binder?
	document private event flag
	implement private flag for MK.Object and MK.Array
	bindSandbox?
	__events and __special should be private
	sandbox and $sandbox properties?
	fire 'remove' when MK.Object#remove is called
	private global event option
	export modules using SystemJS
...
domEventsMap = {
	...
	rem: function( o ) {
		...
		for( i = 0; i < evts.length; i++ ) {
			...
			o.instance.off( '_runbindings:' + o.key, evt.mkHandler );
				...
				if( key ) {
					...
					domEvtHandler = function( evt ) {
						...
						if( selector ) {
							...
							this.removeAttribute( randomID, randomID );
						} else {
							handler.call( this );
						}
					};
					...
					_this._on( 'bind:' + key, f );
					f();
					_this._on( 'unbind:' + key, f2 );
				}
			}
		}
		return _this;
	...
	prototype = {
		...
		_renderOne: function( item ) {
			...
			if( !bound ) {
				...
				if( item && item.isMK && item.bindRendered && $el.length ) {
